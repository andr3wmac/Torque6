...
vec3 lit(vec3 color, vec3 normal, float metalness, float glossiness, vec3 view, vec3 lightDir)
{
	vec3 v = view;
	...
	vec3 cenv = textureCubeLod(u_ambientCube, cubeR, mipLevel).xyz;
	...
	vec3 ambdiff = cd * textureCube(u_ambientIrrCube, cubeN).xyz;
...
vec3 calcPointLight(vec3 wpos, vec3 normal, vec3 view, vec3 light_pos, vec3 light_color, float light_radius, float light_attn)
{
    ...
    vec3 lc = lit(color, normal, 0.2, 0.8, view, lightDir);
...
vec3 calcDirectionalLight(vec3 color, 
                          ...
                          vec3 lightAmbient)
{
    vec3 lc = lit(color, normal, metallic, 1.0 - roughness, view, lightDir) * lightColor;
    vec3 color_out = lc + lightAmbient;
    return color_out;
}
